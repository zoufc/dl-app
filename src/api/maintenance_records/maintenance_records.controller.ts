...
export class MaintenanceRecordsController {
  ...
  async create(
    ...
    try {
      ...
      const maintenanceRecord = await this.maintenanceRecordsService.create(
      ...
      logger.error(
        `---MAINTENANCE_RECORDS.CONTROLLER.CREATE ERROR ${error}---`,
      );
      return res
  ...
  @Get()
  async findAll(@Query() query: FindMaintenanceRecordDto, @Res() res) {
    try {
      logger.info(`---MAINTENANCE_RECORDS.CONTROLLER.FIND_ALL INIT---`);
      const result = await this.maintenanceRecordsService.findAll(query);
      ...
      logger.error(
        `---MAINTENANCE_RECORDS.CONTROLLER.FIND_ALL ERROR ${error}---`,
      );
      return res
        .status(error.status || HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: error.message });
  ...
  async findOne(@Param('id') id: string, @Res() res) {
    try {
      ...
      const maintenanceRecord = await this.maintenanceRecordsService.findOne(
        id,
      );
      ...
      logger.error(
        `---MAINTENANCE_RECORDS.CONTROLLER.FIND_ONE ERROR ${error}---`,
      );
      return res
        .status(error.status || HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: error.message });
  ...
  async update(
    ...
    try {
      ...
      const updated = await this.maintenanceRecordsService.update(
        id,
      ...
      logger.error(
        `---MAINTENANCE_RECORDS.CONTROLLER.UPDATE ERROR ${error}---`,
      );
      return res
        .status(error.status || HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: error.message });
  ...
  async remove(@Param('id') id: string, @Res() res) {
    try {
      ...
      logger.error(
        `---MAINTENANCE_RECORDS.CONTROLLER.REMOVE ERROR ${error}---`,
      );
      return res
        .status(error.status || HttpStatus.INTERNAL_SERVER_ERROR)
        .json({ message: error.message });
