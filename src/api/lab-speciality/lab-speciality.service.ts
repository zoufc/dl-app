...
export class LabSpecialityService {
  constructor(@InjectModel('LabSpeciality') private labSpecialityModel:Model<LabSpeciality>){}
  ...
  async create(createLabSpecialityDto: CreateLabSpecialityDto) {
    try {
      ...
      if(existLabSpeciality) {
        throw new HttpException('Cette spécialité existe déjà pour ce laboratoire', HttpStatus.CONFLICT);
      }
      const labSpeciality = await this.labSpecialityModel.create(createLabSpecialityDto);
  ...
  async findAll() {
    try {
      ...
      const labSpecialities = await this.labSpecialityModel.find()
        .populate('lab', 'structure')
        .populate('speciality', 'name')
        .sort({ created_at: -1 })
        .exec();
      logger.info(`---LAB_SPECIALITY.SERVICE.FIND_ALL SUCCESS---`);
      return labSpecialities;
    } catch (error) {
  ...
  async findOne(id: string) {
    try {
      ...
      const labSpeciality = await this.labSpecialityModel.findById(id)
        .populate('lab', 'structure')
        .populate('speciality', 'name')
        .exec();
      if (!labSpeciality) {
        throw new HttpException('Spécialité de laboratoire non trouvée', HttpStatus.NOT_FOUND);
      }
  ...
  async update(id: string, updateLabSpecialityDto: UpdateLabSpecialityDto) {
    try {
      ...
      const updated = await this.labSpecialityModel.findByIdAndUpdate(
        id,
  ...
  async remove(id: string) {
    try {
      ...
      const deleted = await this.labSpecialityModel.findByIdAndDelete(id).exec();
      if (!deleted) {
        throw new HttpException('Spécialité de laboratoire non trouvée', HttpStatus.NOT_FOUND);
      }
