...
export class InventoryTransactionsController {
  ...
  @Post()
  async create(@Body() createInventoryTransactionDto: CreateInventoryTransactionDto, @Res() res) {
    try {
      logger.info(`---INVENTORY_TRANSACTIONS.CONTROLLER.CREATE INIT---`);
      const inventoryTransaction = await this.inventoryTransactionsService.create(createInventoryTransactionDto);
      ...
      logger.error(`---INVENTORY_TRANSACTIONS.CONTROLLER.CREATE ERROR ${error}---`);
  ...
  async findAll(@Res() res) {
    try {
      ...
      const inventoryTransactions = await this.inventoryTransactionsService.findAll();
      ...
      logger.error(`---INVENTORY_TRANSACTIONS.CONTROLLER.FIND_ALL ERROR ${error}---`);
      return res.status(error.status || HttpStatus.INTERNAL_SERVER_ERROR).json(error);
    }
  }
  ...
  async findOne(@Param('id') id: string, @Res() res) {
    try {
      ...
      const inventoryTransaction = await this.inventoryTransactionsService.findOne(id);
      ...
      logger.error(`---INVENTORY_TRANSACTIONS.CONTROLLER.FIND_ONE ERROR ${error}---`);
      return res.status(error.status || HttpStatus.INTERNAL_SERVER_ERROR).json(error);
    }
  }

  @Patch(':id')
  async update(@Param('id') id: string, @Body() updateInventoryTransactionDto: UpdateInventoryTransactionDto, @Res() res) {
    try {
      logger.info(`---INVENTORY_TRANSACTIONS.CONTROLLER.UPDATE INIT---`);
      const updated = await this.inventoryTransactionsService.update(id, updateInventoryTransactionDto);
      ...
      logger.error(`---INVENTORY_TRANSACTIONS.CONTROLLER.UPDATE ERROR ${error}---`);
      return res.status(error.status || HttpStatus.INTERNAL_SERVER_ERROR).json(error);
    }
  }
  ...
  async remove(@Param('id') id: string, @Res() res) {
    try {
      ...
      logger.error(`---INVENTORY_TRANSACTIONS.CONTROLLER.REMOVE ERROR ${error}---`);
      return res.status(error.status || HttpStatus.INTERNAL_SERVER_ERROR).json(error);
    }
  }
}
