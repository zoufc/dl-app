...
export class DistrictController {
  ...
  async create(@Body() createDistrictDto: CreateDistrictDto,@Res() res) {
    try {
      ...
      return res.status(HttpStatus.CREATED).json({message:"District créé",data:district})
    } catch (error) {
  ...
  async createMany(@Res() res) {
    try {
      ...
      return res.status(HttpStatus.CREATED).json({message:"Districts créés",data:districts})
    } catch (error) {
  ...
  @Patch(':id')
  async update(@Param('id') id: string, @Body() updateDistrictDto: UpdateDistrictDto, @Res() res) {
    try {
